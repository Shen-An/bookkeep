...
- **设置页面**：全局设置页，包含通用设置（货币单位选择 ¥$€£）、AI 配置入口、数据管理（清除数据）、关于信息
- **AI 配置页面**：在应用内直接编辑文字识别和图片识别的 API Base URL / Key / Model，无需修改配置文件
- **开源信息页面**：展示项目开源许可信息

> ⚠️ 货币单位选择目前仅在设置页保存，首页账单列表暂未跟随切换（仅 ¥ 显示）。
...
第一阶段（P1）    → 预算管理 + 暗黑模式 + 手势优化
第二阶段（P2）    → 多账户 + 周期账单 + 统计增强
第三阶段（P3）    → 语音记账 + 储蓄目标 + 附件
...
- 设置
    - 货币单位选择已保存到 Preferences，但首页账单列表、经济图表等页面尚未跟随切换（需读取 SettingManager 全局应用）
